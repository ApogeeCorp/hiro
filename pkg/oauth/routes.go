...
func Routes(ctrl Controller) []api.Route {
	...
	return []api.Route{
		...
		api.NewRoute("/token").Post().Handler(token).Context(ctrl).Validate(),
		api.NewRoute("/token-introspect").Post().Handler(tokenIntrospect).Context(ctrl).Validate().Authorizers(auth.AuthorizeScope(ScopeTokenRead)),
		api.NewRoute("/token-revoke").Post().Handler(tokenRevoke).Context(ctrl).Validate().Authorizers(auth.AuthorizeScope(ScopeTokenRevoke)),
